      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[#f0e6d2] text-[#2c1810] pattern-bg">
        <div className="max-w-md w-full bg-[#fffcf5] p-10 rounded-xl shadow-2xl border-4 border-double border-[#8b1c1c]/40 text-center">
          ...
          <p className="mb-8 text-lg opacity-80 italic">
            ...
            Trasforma il tuo volto in un personaggio delle carte tradizionali.
          </p>
          <button
            onClick={handleKeySelection}
            className="w-full py-4 bg-[#8b1c1c] hover:bg-[#6b1616] text-[#fffcf5] vintage-font tracking-widest text-lg font-bold rounded shadow-lg border border-[#4a0e0e] transition-all"
          >
            ENTRA NELLA FIERA
          </button>
          <p className="mt-6 text-sm text-[#8b5a2b] font-serif italic">
            Richiede una chiave API Google Cloud valida.
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen font-serif pb-20">
      ...
      <header className="bg-[#2c1810] text-[#f0e6d2] p-6 shadow-xl sticky top-0 z-50 border-b-4 border-[#b8860b]">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          {croppedBase64 && selectedCard && !isLoading && !generatedImage && (
             <button
             onClick={handleGenerate}
             className="md:hidden bg-[#b8860b] text-[#2c1810] px-4 py-2 rounded font-bold vintage-font shadow-md border border-[#f0e6d2]/20"
           >
             Crea
           </button>
          )}
